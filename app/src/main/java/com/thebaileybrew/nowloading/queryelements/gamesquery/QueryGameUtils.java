    ...
    public static void extractDataFromJson(String jsonString, Context context) {
        ...
        try {
            ...
            for (int g = 0; g < baseJSONArray.length(); g++) {
                ...
                if (currentGame.getString("DEV HOME COUNTRY").isEmpty()) {
                    gameDevCountry = "Developer Country Unknown";
                } else {
                    gameDevCountry = currentGame.getString("DEV HOME COUNTRY");
                }
                gameDevPhone = currentGame.getString("DEV PHONE");
                //check for Dev Establish Date
                if (currentGame.getString("DEV ESTABLISHED").isEmpty()) {
                    gameDevEstablished = "Developer Establish Date Unknown";
                } else {
                    gameDevEstablished = currentGame.getString("DEV ESTABLISHED");
                }
                gameGenre = currentGame.getString("GENRE");
                gameReleased = currentGame.getString("RELEASE DATE");
